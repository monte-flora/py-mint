...
class AccumulatedLocalEffects(object):
    """
    ...
    def calculate_second_order_ale(self, feature=None, **kwargs):
        ...
        if self._x1vals is None:
            self._x1vals = np.percentile(
                self._examples[feature[0]].values, np.arange(2.5, 97.5 + 5, 5))

        if self._x2vals is None:
            self._x2vals = np.percentile(
                self._examples[feature[1]].values, np.arange(2.5, 97.5 + 5, 5))
